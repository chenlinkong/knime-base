...
public final class IndividualTableReaderFactory<I, T, V> {
    ...
    public DefaultIndividualTableReader<I, V> create(final I item, final FileStoreFactory fsFactory) {
        ...
        final DataCell identifierCell = createIdentifierCell(item);
        return new DefaultIndividualTableReader<>(typeMapper, idxMapper, rowKeyGen, identifierCell);
    }

    private DataCell createIdentifierCell(final I item) {
        if (m_itemIdentifierCellFactory == null) {
            return null;
        } else {
            return m_itemIdentifierCellFactory.apply(item);
        }
    }
