...
final class KNIMERemoteFileSystem extends BaseFileSystem<KNIMERemotePath> {
    ...
    private static URI createURI(final KNIMERemoteFSConnectionConfig config) {
        try {
            return new URI(FS_TYPE.getTypeId(), config.getMountpoint(), null, null);
        } catch (URISyntaxException ex) {
            throw new IllegalArgumentException("Illegal mountpoint: " + config.getMountpoint(), ex);
        }
    }
