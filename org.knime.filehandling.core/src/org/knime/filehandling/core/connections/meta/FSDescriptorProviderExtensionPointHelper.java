...
import org.knime.filehandling.core.connections.knimerelativeto.FSConnectionProvider;
...
final class FSDescriptorProviderExtensionPointHelper {
     ...
     */
    static synchronized Map<FSType, FSDescriptor> loadRegisteredFSDescriptors() {
        final Map<FSType, FSDescriptor> descriptors = new HashMap<>();
    ...
    private static void addDescriptorIfPossible(final IConfigurationElement elem, final Map<FSType, FSDescriptor> descriptors) {
        ...
        if (StringUtils.isBlank(className)) {
            ...
            try {
                ...
                if (descriptors.containsKey(provider.getFSType())) {
                    LOGGER.error(String.format("FSDescriptor with FSType %s has already been registered. Ignoring extension with id '%s'.", //
                        provider.getFSType(), //
                        decl));
                } else {
                    descriptors.put(provider.getFSType(), provider.getFSDescriptor());
                }
