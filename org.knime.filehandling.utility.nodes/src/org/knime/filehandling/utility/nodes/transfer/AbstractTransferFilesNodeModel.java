...
public abstract class AbstractTransferFilesNodeModel<T extends AbstractTransferFilesNodeConfig> extends NodeModel {
     ...
     *
     * @param inSpecs the input specs
     * @return the source location's spec
     */
    protected abstract FSLocationSpec getSrcLocationSpecs(final PortObjectSpec[] inSpecs);

    /**
     * Returns the target location's {@link FSLocationSpec}.
     *
     * @param inSpecs the input specs
     * @return the target location's spec
     */
    protected FSLocationSpec getDestLocationSpecs(final PortObjectSpec[] inSpecs) {
        return getConfig().getDestinationFileChooserModel().getLocation();
    }

    /**
     * Creates a {@link DataColumnSpec} for the source or destination path which includes the meta data.
     *
     * @param sourceLocationSpec the location spec specifying the file system
     * @param columnName the name of the column
     * @return the {@link DataColumnSpec}
     */
    private static DataColumnSpec createMetaColumnSpec(final FSLocationSpec locationSpec, final String columnName) {
        final FSLocationValueMetaData metaData = new FSLocationValueMetaData(locationSpec.getFileSystemCategory(),
            locationSpec.getFileSystemSpecifier().orElse(null));
