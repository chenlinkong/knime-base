...
final class PathCopier2 {
    ...
    private DataCell[][] handleMissingSrc(final ExecutionContext exec, final TransferEntry entry)
        ...
        if (m_failIfSrcDoesNotExist) {
            ...
            exec.setProgress(1);
            return new DataCell[][]{createSrcDoesNotExistRow(exec, entry.getSource())};
        }
    }
    ...
    private int copy(final ExecutionContext exec, final DataCell[][] rows, final int idx, final TransferPair p,
        ...
        final DataCell[] row = copyPath(exec, p.getSource(), p.getDestination());
        if (verbose) {
            rows[idx] = row;
        }
        return idx + 1;
     ...
     */
    private DataCell[] copyPath(final ExecutionContext exec, final FSPath src, final FSPath dest) throws IOException {
        validatePair(src, dest);

        final DataCell[] cells = new DataCell[NUMBER_OF_DEFAULT_COLS];
        cells[SOURCE_COL_IDX] = m_sourceFSLocationCellFactory.createCell(exec, src.toFSLocation());
        cells[DESTINATION_COL_IDX] = m_destinationFSLocationCellFactory.createCell(exec, dest.toFSLocation());
     ...
     */
    private DataCell[] createSrcDoesNotExistRow(final ExecutionContext exec, final FSPath src) {
