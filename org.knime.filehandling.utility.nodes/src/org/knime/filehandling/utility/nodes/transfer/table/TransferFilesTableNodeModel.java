...
final class TransferFilesTableNodeModel extends AbstractTransferFilesNodeModel<TransferFilesTableNodeConfig> {
    ...
    @Override
    protected FSLocationSpec getSrcLocationSpecs(final PortObjectSpec[] inSpecs) {
    ...
    private FSLocationSpec getLocationSpec(final PortObjectSpec[] inSpecs, final int connectionIdx,
        final String colName) {
        if (connectionIdx >= 0) {
            return ((FileSystemPortObjectSpec)inSpecs[connectionIdx]).getFSLocationSpec();
        } else {
            return getLocationSpecs(inSpecs, colName);
        }
    }

    private FSLocationSpec getLocationSpecs(final PortObjectSpec[] inSpecs, final String pathColName) {
        ...
        return FSLocationColumnUtils.validateAndGetMetaData(fsLocationColSpec);
    }
